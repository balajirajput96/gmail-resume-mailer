...
vi.mock("./db", () => ({
  getDb: vi.fn(async () => ({
    ...
    insert: () => ({ values: (input: Record<string, unknown>) => ({ onDuplicateKeyUpdate: async () => { state.inserted.push(input); state.row = input; } }) }),
  })),
}));
...
describe("GitHub OAuth encrypted persistence", () => {
  it("stores an encrypted access token and returns the connection without plaintext storage", async () => {
    ...
    await saveGitHubConnection({ userId: 7, githubLogin: "balajirajput96", accessTokenCiphertext, scopes: "repo read:user" });

    expect(state.inserted).toHaveLength(1);
    expect(state.inserted[0].accessTokenCiphertext).toBe(accessTokenCiphertext);
    expect(String(state.inserted[0].accessTokenCiphertext)).not.toContain(token);
    expect(decryptServerSecret(accessTokenCiphertext)).toBe(token);
    await expect(getGitHubConnection(7)).resolves.toMatchObject({ userId: 7, githubLogin: "balajirajput96", accessTokenCiphertext });
  });
});
