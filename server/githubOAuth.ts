...
function createState(userId: number) { const payload = Buffer.from(JSON.stringify({ userId, nonce: crypto.randomBytes(24).toString("base64url"), exp: Date.now() + 10 * 60 * 1000 })).toString("base64url"); return `${payload}.${signValue(payload)}`; }
...
async function exchangeCode(code: string, redirect: string) { const { clientId, clientSecret } = config(); const response = await fetch("https://github.com/login/oauth/access_token", { method: "POST", headers: { Accept: "application/json", "Content-Type": "application/x-www-form-urlencoded" }, body: new URLSearchParams({ client_id: clientId, client_secret: clientSecret, code, redirect_uri: redirect }) }); if (!response.ok) throw new Error("GitHub OAuth exchange failed"); const payload = await response.json() as { access_token?: string; scope?: string; error?: string }; if (!payload.access_token || payload.error) throw new Error("GitHub OAuth response is incomplete"); return payload; }

export function registerGitHubOAuthRoutes(app: Express) {
  app.get("/api/github/oauth/start", async (req, res) => { try { const user = await sdk.authenticateRequest(req); const state = createState(user.id); const parsed = parseState(state); res.cookie(STATE_COOKIE, parsed?.nonce ?? "", { httpOnly: true, secure: ENV.isProduction, sameSite: "lax", maxAge: 10 * 60 * 1000, path: "/api/github/oauth" }); const url = new URL("https://github.com/login/oauth/authorize"); url.search = new URLSearchParams({ client_id: config().clientId, redirect_uri: redirectUri(req), scope: GITHUB_SCOPES.join(" "), state }).toString(); res.redirect(url.toString()); } catch { res.status(503).send("GitHub connection is not configured yet."); } });
  app.get("/api/github/oauth/callback", async (req, res) => { const baseUrl = appBaseUrl(req); try { const state = typeof req.query.state === "string" ? parseState(req.query.state) : null; const code = typeof req.query.code === "string" ? req.query.code : null; const cookies = parseCookies(req.headers.cookie ?? ""); if (!state || !code || cookies[STATE_COOKIE] !== state.nonce) throw new Error("OAuth state validation failed"); const token = await exchangeCode(code, redirectUri(req)); const accessToken = token.access_token; if (!accessToken) throw new Error("GitHub OAuth response is incomplete"); await saveGitHubConnection({ userId: state.userId, githubLogin: await fetchGitHubLogin(accessToken), accessTokenCiphertext: encryptServerSecret(accessToken), scopes: token.scope || GITHUB_SCOPES.join(" ") }); res.clearCookie(STATE_COOKIE, { path: "/api/github/oauth" }); res.redirect(`${baseUrl}/?github=connected`); } catch { res.clearCookie(STATE_COOKIE, { path: "/api/github/oauth" }); res.redirect(`${baseUrl}/?github=error`); } });
}
