...
## Ai-agent-hub Maintenance Recovery — 20 August 2026

The `Daily Maintenance` workflow failed in run `32411483552` because its migration metadata check did not receive `DATABASE_URL`, even though the check itself does not open a database connection. The repair in `ai-agent-hub` commit `b66ef50` gives only that metadata check a non-secret local fallback URL when the environment is absent. The full maintenance command passed locally with `DATABASE_URL` unset, including formatting, typecheck, tests, build, migration validation, audit, and diff checks. The GitHub replacement run `32412141065` completed successfully.
