...
function Workspace() {
  ...
  const submitPlan = () => {
    if (!selectedRepositoryId) { setPlanError("Choose a repository first"); toast.error("Choose a repository first"); return; }
  ...
  return <div className="mx-auto max-w-7xl pb-12">
    ...
    <div className="grid gap-5 xl:grid-cols-[1.05fr_.95fr]">
      <div className="space-y-5">
        ...
        <section className="rounded-3xl border border-[#e3e6ef] bg-white p-6 shadow-[0_20px_55px_-38px_rgba(30,37,65,.45)]">
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            {repositories.length === 0 ? <div className="col-span-full rounded-2xl border border-dashed border-[#d7dcef] bg-[#fafbff] p-6 text-sm text-muted-foreground">Import public repositories or add one manually below.</div> : repositories.map(repository => <button key={repository.id} type="button" onClick={() => setSelectedRepositoryId(current => current === repository.id ? null : repository.id)} aria-pressed={selectedRepositoryId === repository.id} title={selectedRepositoryId === repository.id ? "Click to deselect this repository" : "Select this repository"} className={`rounded-2xl border p-4 text-left transition-all ${selectedRepositoryId === repository.id ? "border-[#6979db] bg-[#f3f5ff] shadow-sm" : "border-[#e5e7ee] bg-white hover:border-[#c9d0ee]"}`}><div className="flex items-center justify-between gap-3"><span className="truncate font-medium text-[#30354d]">{repository.fullName}</span><span className={`h-2 w-2 shrink-0 rounded-full ${selectedRepositoryId === repository.id ? "bg-[#6979db]" : "bg-[#d7dbe8]"}`} /></div><p className="mt-2 line-clamp-2 min-h-10 text-xs leading-5 text-muted-foreground">{repository.description || "No description provided"}</p><div className="mt-3 flex items-center gap-2 text-[11px] text-[#69718c]"><span>{repository.visibility}</span><span>•</span><span>{repository.defaultBranch}</span></div></button>)}</div>
      ...
      <div className="space-y-5">
        ...
        <section className="rounded-3xl border border-[#e3e6ef] bg-white p-6 shadow-[0_20px_55px_-38px_rgba(30,37,65,.45)]"><div className="flex items-center gap-3"><span className="grid h-10 w-10 place-items-center rounded-2xl bg-[#fff0e9] text-[#bf6547]"><ImagePlus className="h-5 w-5" /></span><div><h2 className="text-xl font-semibold text-[#25293e]">Creative asset</h2><p className="text-sm text-muted-foreground">GPT Image 2, medium quality, generated on demand.</p></div></div><Textarea className="mt-5 min-h-24" value={imagePrompt} onChange={event => { setImagePrompt(event.target.value); setImageError(null); }} aria-label="Creative asset prompt" aria-describedby={imageError ? "image-prompt-error" : undefined} /><Button className="mt-3 w-full bg-[#bf6547] hover:bg-[#a9533a]" disabled={generateImage.isPending} onClick={submitImage}>{generateImage.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <ImagePlus className="mr-2 h-4 w-4" />}{generateImage.isPending ? "Generating…" : "Generate image"}</Button>{imageError ? <p id="image-prompt-error" role="alert" className="mt-3 text-sm font-medium text-rose-700">{imageError}</p> : null}{media.data?.length ? <div className="mt-5 grid grid-cols-3 gap-2">{media.data.slice(0, 6).map((asset: { id: number; assetUrl: string; prompt: string }) => <a key={asset.id} href={asset.assetUrl} target="_blank" rel="noreferrer" className="group overflow-hidden rounded-xl border border-[#ece6e2] bg-[#faf7f5]"><img src={asset.assetUrl} alt={asset.prompt} className="aspect-square w-full object-cover transition-transform duration-200 group-hover:scale-105" /></a>)}</div> : null}</section>
      </div>
    </div>
  </div>;
}
